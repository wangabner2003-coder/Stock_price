...
The datasets are stored separately because the generated CSV files are too large for a standard GitHub repository:

- [Download the project data from Google Drive](https://drive.google.com/drive/folders/1sRcA7MMLqWpeiyMxbVkKgbqbYI9x3wse?usp=sharing)

The shared folder should contain:

- `all_stocks_with_features.csv`
- `all_stocks_targets.csv`
- The `Pure_data/` folder with raw OHLCV files for all 11 assets

Download these files and place them in the repository structure shown above before running the notebooks. Ensure the Google Drive folder is shared with **Anyone with the link — Viewer** so other users can access it.
